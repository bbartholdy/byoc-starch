---
title: "Investigating Biases Associated with Dietary Starch Incorporation into an Oral Biofilm Model"
author:
...
calculus in -0.5 M<!-- what percentage???--> ethylenediaminetetraacetic acid (EDTA) 
...
```{r amylase-activity-tbl}
rownames(plt1_ph1_result) <- rows
colnames(plt1_ph1_result) <- cols
knitr::kable(signif(plt1_ph1_result, 3), row.names = TRUE, caption = "Amylase activity in U/mL enzyme, where a U is mg maltose released from starch in six minutes at 36 degrees Celsius. Plate 1, photometric reading 1. Negative values converted to 0.")
```
...
The mixed treatment had the highest absolute count of starch granules in solution (mean = `r signif(subset(count_sol, solution == "mix")$total, 3)`),
while the biofilms exposed to the wheat solution preserved the greatest number of
granules 
(mean = `r signif(subset(count_sample, treatment == "wheat")$total, 3)`). 
The potato treatment had the lowest absolute counts in both the solution 
(`r signif(subset(count_sol, solution == "potato")$total, 3)`)
and in the biofilm samples 
(`r signif(subset(count_sample, treatment == "potato")$total, 3)`) 
...
```{r sep-plot, fig.cap="(A) Proportion (%) of sizes of starch granules in the wheat solution (outer ring) and extracted from the wheat-treatment samples (inner ring), and (B) in the potato solution (outer ring) and extracted from the potato-treatment samples (inner ring)."}
...
```{r mix-plot, fig.cap="Proportion of sizes of (A) wheat granules in the mixed solution (outer ring) and extracted from the wheat-treatment samples (inner ring), and sizes of (B) potato granules in the solution (outer ring) and extracted from the potato-treatment samples (inner ring)."}
...
(mean = `r format(count_sol$total[4], big.mark = ",")`) 
and potato
(mean = `r format(count_sol$total[3], big.mark = ",")`)
solutions.  
...
The size bias is consistent with the findings by Power and colleagues
[-@powerChimpCalculus2015],
which found that plants producing starches 10--20 $\mu$m in
size were over-represented; although, the representation of granules larger than
20 $\mu$m is unclear.  
Given the differences in proportional incorporation between wheat and potato,
especially for small and medium granules, it is also possible that the morphology
of granules plays a role. However, the proportional count differences between large 
wheat and potato granules were less pronounced.

<!-- Since those results were obtained -->
<!-- on a modern population of chimps that were only buried for a short period of time,  -->
<!-- degradation was likely limited [@powerChimpCalculus2015]. -->

The taphonomy of starch granules has been addressed previously, and depends on 
burial environment (pH, temperature, moisture content, microorganisms), as well 
as processing of the granules prior to deposition 
[@henryCookingStarch2009; @graneroStarchTaphonomy2020; @haslamDecompositionStarch2004]. 
...
<!-- add potential for morphological differences affecting incorporation 
 ...
 Colonisation of bacteria onto starch @nagaraBacteriaNutrients2017-->
...
from our results is the size of the calculus deposit, which seems to influence the 
quantity of starches extracted from the calculus, as we found a strong positive 
correlation between biofilm size and retained starch granules (Figure \@ref(fig:cor-plot));
...
in archaeological contexts could be due to diagenetic effects. Alternately, the absence of 
amylase activity in our study could be impacting our interpretations.

<!-- How are starch granules trapped in dental calculus? Does anyone actually know? - adhesion to bacteria
- stuck in cracks and crevices
- covered by new layer of plaque
...
during mastication may be retained in the dentition. 

<!-- initial incorporation of starch granules? 
cracks and crevices? attachment to bacteria? -->
...
a new layer of plaque has covered the surface of the calculus. 
...
<!--Bacteria also have the ability to adhere to starch [@toppingResistantStarch2003],
...
clearance. -->
<!-- Unmineralised lacunae and channels within the  
...
with our results 
(Figures \@ref(fig:cor-plot) and \@ref(fig:cor-plot2)).
...
(10--20 $\mu$m)
starches an increased rate, and small (<10 $\mu$m) granules remained somewhat constant.
