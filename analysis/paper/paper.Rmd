...
assay was conducted to confirm that no amylase was present in the system following
starch treatments. Starch treatments replaced sucrose treatments, occurring twice per day
...
The substrata were submerged in 1 ml/well CPMU five times daily, 
...
sucrose as a treatment, were included to detect starch contamination.
...
All samples and standard curves were added in triplicates to two separate plates.
...
maltose released from starch by $\alpha$-amylase activity.
...
When dispersing the liquid (EDTA) sample on the slide, the sample was homogenised using 
the pipette. It was not possible to count all wheat starches
on a slide, so wheat and mix counts were extrapolated as described for the treatment
solutions.

## Statistical analysis

Statistical analysis was conducted in `r version$version.string`  [@R-base].
<!-- R packages used -->
...
All scripts and data are available on OSF (https://osf.io/uc5qy/)
and Github (https://github.com/bbartholdy/byoc-starch).
...
in a controlled laboratory setting. The addition of known starch species in
...
investigate various aspects of dietary research and starch incorporation and
retention in dental calculus.
